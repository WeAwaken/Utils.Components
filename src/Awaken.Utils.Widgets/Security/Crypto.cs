...
namespace Awaken.Utils.Widgets
{
    public static class Crypto
    {
        ...
        private static HashAlgorithm Create(int hashSize)
        {
            switch (hashSize)
            {
                case 256:
        ...
        public static string Hash(string input, int algorithm = 256)
        {
            if (input == null)
            {
        ...
        public static string Hash(byte[] input, int algorithm = 256)
        {
            if (input == null)
            {
                throw new ArgumentNullException("input");
        ...
        /// <param name="isUpper">是否输出大写</param>
        /// <returns></returns>
        public static string MD5Encrypt(string source, bool isUpper)
        {
            var md5 = MD5.Create();

            var bytes = Encoding.UTF8.GetBytes(source);

            var hs = md5.ComputeHash(bytes);
